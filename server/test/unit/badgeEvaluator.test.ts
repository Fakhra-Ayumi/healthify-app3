...
describe('Unit: Badge Evaluator', () => {
  ...
  before(async () => {
    ...
    await Badge.insertMany([
      { name: 'Weekly Bronze', description: 'Complete weekly goal twice', icon: 'EmojiEvents', criteriaType: 'weekly_goal', criteriaValue: 2, tier: 'bronze' },
      { name: 'Weekly Silver', description: 'Complete weekly goal 6 times', icon: 'EmojiEvents', criteriaType: 'weekly_goal', criteriaValue: 6, tier: 'silver' },
  ...
  it('should NOT duplicate an already-earned badge', async () => {
    const user = await evaluateBadges(userId);
    const count = user!.badges.filter((b: string) => b === 'Weekly Bronze').length;
